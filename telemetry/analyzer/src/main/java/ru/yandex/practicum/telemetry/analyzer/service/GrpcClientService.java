...
public class GrpcClientService {
    ...
    public void handleScenario(Scenario scenario) {
        log.trace("Поступил сценарий {} для отправки хабу {}", scenario.getName(), scenario.getHubId());
        ...
        for (Map.Entry<String, Action> actionEntry : scenario.getActions().entrySet()) {
            ...
            if (scenarioAction.getType().equals(ActionTypeAvro.SET_VALUE) && scenarioAction.getValue() != null) {
                actionBuilder.setValue(scenarioAction.getValue());
            }

            try {
                log.info("Отправка сообщения хабу {} для сценария {} и датчика {}",
                ...
                log.error("Ошибка при отправке хабу {} действия {} для устройства {}",
                        scenario.getHubId(), scenarioAction.getType(), sensorId, e);
            }
        }
    }
}